...
class SecretScanner(BaseScanner):
    ...
    def scan(self, data: Dict[str, Any]) -> List[Finding]:
        ...
        for repo in data.get("github_repos", []):
            self._scan_text(repo.get("name", ""), f"github_repo:{repo.get('name')}", ResourceType.REPO, findings)
            
        return findings

    def _scan_text(self, text: str, resource_id: str, resource_type: ResourceType, findings: List[Finding]):
